...
`check_routes.py` 会同时检查本地代码路由和实际运行服务路由。若本地代码有接口、实际服务缺失，通常是旧的 `uvicorn`/`python` 进程仍在占用端口，或使用了错误 Python 环境启动后端；停止旧进程后用项目 `.venv` 重新启动即可。
...
- `/`：商品列表首页，顶部为“发现好物”和搜索框，下方为平台轮播图、紧凑分类筛选、关键词、价格、排序、分页和商品卡片；轮播图提供左右切换按钮。
- `/login`：独立用户登录页。
- `/register`：独立用户注册页。
- `/products/:productId`：商品详情，含图片切换、SKU、价格、销量、评价、收藏、店铺入口、商家客服入口、普通加购和拼团入口。
- `/group-buy`：拼团专区。
- `/cart`：购物车。
- `/checkout`：结算和支付宝支付。
- `/orders`：订单列表、状态筛选、分页和快捷操作，查看详情后进入订单详情页；列表页不再直接展示支付信息和售后记录。
- `/orders/:orderId`：订单详情，含流转时间、商品明细、收货地址、物流信息、优惠信息、支付信息、评价商品、申请售后，以及商家客服/平台客服弹窗入口。
- `/community`：社区广场、分区、话题、发帖、评论、点赞、收藏和种草来源。
- `/customer-service`：客服消息兼容页。
- `/user`：用户中心，含资料、头像、地址、优惠券、积分会员、收藏商品、关注店铺、收藏帖子和客服消息板块。
- `/merchants/:merchantId`：用户端店铺主页。
- `/products`、`/promotions`、`/addresses`：兼容旧路径，重定向到当前正式入口。
...
当前管理端已经拆为平台后台和商家后台正式页面。主要路由：

- `/`：管理入口。
- `/admin/login`：平台登录。
- `/admin/dashboard`：平台数据概览。
- `/admin/home-banners`：首页轮播配置。
- `/admin/merchant-review`：商家入驻审核。
- `/admin/category`：分类配置。
- `/admin/products`：商品监管。
- `/admin/orders`：全平台订单。
- `/admin/refunds`：售后处理。
- `/admin/coupons`：促销管理与会员积分配置；会员等级可配置签到加成、单笔积分抵扣上限覆盖、积分倍率预留值和权益说明。
- `/admin/community`：社区内容管理。
- `/admin/customer-service`：平台客服会话工作台，只处理平台咨询。
- `/admin/users`：用户管理。
- `/merchant/login`：商家登录。
- `/merchant/dashboard`：商家数据概览。
- `/merchant/products`：商品管理。
- `/merchant/orders`：本店订单。
- `/merchant/refunds`：本店售后。
- `/merchant/coupons`：本店优惠券。
- `/merchant/full-discounts`：本店满减活动。
- `/merchant/group-buy`：拼团配置。
- `/merchant/community`：社区动态。
- `/merchant/customer-service`：本店客服会话工作台，只处理本店咨询。
- `/merchant/store`：店铺信息。
- `/onboarding`：商家自助入驻。
...
5. 平台在 `/admin/home-banners` 配置首页轮播图：上传图片，填写标题/副标题，选择不跳转、跳转商品或跳转链接；跳转商品通过可搜索商品下拉框选择，不需要手填商品 ID；再设置排序和启停。用户端首页只展示启用轮播图。上传接口返回 `/static/uploads/...`，后端会把文件写入并静态挂载 `backend/uploads`。平台在 `/admin/category` 创建、编辑、停用或查看分类；分类 ID、父级关系、完整路径和排序应直接展示。分类最多三级，排序只影响同一父级下展示顺序。停用分类为软停用，有启用子分类或已有商品占用时不能停用。
...
7. 用户在用户端注册/登录，编辑个人资料并上传头像，新增/修改/删除带街道、邮编、标签的地址，浏览商品。首页应展示平台启用轮播图；分类筛选保持紧凑胶囊样式，默认只展示一级分类，选中父分类后才展开下一级，没有子分类则不显示下级区域；用户端分类区域只展示名称，不展示分类 ID。选择父级分类时，应能看到该父级及全部子分类商品；商品列表支持关键词、价格区间和排序筛选；商品卡片、商品详情和店铺页应显示销售价与划线价；商品详情应展示评分摘要和公开评价列表。点击商品卡片或商品详情中的店铺 ID/店铺名，应进入 `/merchants/:merchantId` 店铺主页并能关注/取消关注店铺、查看关注数、领取店铺可用优惠券、筛选价格、切换排序、进入商品详情和加入购物车；商品详情应能收藏/取消收藏商品，回到用户端首页后，应能在“我关注的店铺”和“我收藏的商品”中看到对应店铺或商品入口。
...
11. 商家或平台在管理端订单详情区发货，商家端列表中的“发货”入口会先打开订单详情；发货时必须选择物流公司并填写或生成模拟单号，不做真实物流轨迹查询。
12. 用户端订单区应支持分页、按订单状态筛选、取消待支付订单、查看订单详情、查看支付单状态、支付宝沙箱支付、确认收货、评价商品和申请售后；订单详情页也必须提供评价和售后入口，不能只放在订单列表卡片里。订单列表不直接放联系客服按钮，进入订单详情后可分别联系商家客服和平台客服，两个入口都应直接打开右下角弹窗并进入该订单对应会话。申请售后时必须选择订单明细和退款数量，申请后应能在“我的售后”中看到售后 ID、订单 ID、订单明细 ID、商品 ID、SKU ID、数量、状态、金额和原因。
13. 用户确认收货后，可选择订单中的具体商品分别用星级、文字和图片评价，也可在社区发布普通帖、体验分享、询问求助或种草帖。
14. 商家可在商家端查看本店订单详情、发货、处理本店售后详情并回复本店客服会话；平台可查看全平台订单详情、查看售后详情和凭证、隐藏不合适的帖子/评论/评价、处理全平台售后、回复平台客服会话，导出订单 CSV，查看操作日志。客服会话状态前端展示为“进行中/已结束”，结束按钮文案为“结束”。
