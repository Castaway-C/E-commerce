...
- 新增后端功能时，同轮补充前端调用入口；现有模块已经具备完整页面，后续不要退回“只做接口、页面以后再接”的做法。
...
- 报表和客服接口也必须遵循权限边界：平台可看全局，商家只能看本店，普通用户只能看自己的数据和会话。
...
- 后续开发按完整功能基线维护，不再交付只有占位、半流程或临时按钮的功能。
...
- 正常前端业务只使用支付宝沙箱扫码支付。
- 不接入微信支付。
- mock 支付只允许作为历史兼容或后端测试兜底，不得出现在正常前端业务入口。
...
- `docs/development-roadmap.md`：后续 AI 助手、验收硬化和展示优化计划。
- `docs/handoff.md`：协作移交说明，接手同学优先阅读。
- `docs/dev-setup.md`：启动、清库、账号、联调和验收说明。
- `AGENTS.md`：AI 协作、项目状态和上下文恢复说明。
- `需求规格说明书.md`、`实现设计说明书.md`：早期历史资料，后续不再作为主要编码依据。
